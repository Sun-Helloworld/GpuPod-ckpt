...
//  DLOG(ERROR, "👀 switch(op) 判断重放哪一个api\n");
  printf("💧  switch(op) 判断重放哪一个api    \n");
  switch(op) {
...
// 
      ...
      printf("💧 cudaMalloc replayed newdevptr=%p,olddevptr = %p \n",newDevPtr,oldDevPtr);
    ...
    case GENERATE_ENUM(__cudaPushCallConfiguration):
    {
      ...
      memcpy(&stream, l->fncargs + chars_read, sizeof (void *));
      // replay
      printf("💧___cudaPushCallConfigurationreplayed \n");
    ...
    case GENERATE_ENUM(__cudaRegisterFatBinary):
    {
      ...
      // replay
      printf("💧 cudaRegisterFatBinary replayed \n");
  ...
  case GENERATE_ENUM(__cudaRegisterFatBinaryEnd):
    {
      ...
      printf("💧 cudaRegisterFatBinaryend replayed \n");
    ...
    case GENERATE_ENUM(__cudaRegisterFunction):
    {
      ...
      //   deviceName, thread_limit, &tid, &bid, &bDim, &gDim, &wSize);
            printf("💧__cudaRegisterFunction replayed \n");
...
{ 
  ...
  // DLOG(ERROR, "enterd logs_read_and_apply \n");
  printf("💧enterd logs_read_and_apply \n");
