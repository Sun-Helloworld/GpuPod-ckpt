...
//    void * cptr=NULL;
 //   cudaMalloc(&cptr, 436*sizeof(char));
  ...
  //  通过dmtcp恢复上半部分内存
    ...
    // 根据pid获取内存信息
    readUhInfoAddr();
    // 读取上半部分中的api记录 进行重放，但是重放好像是取得了新的地址
    logs_read_and_apply();
  ...
  for (int i = 0; i < total_entries; i++) {
    ...
    switch (lhpage_info.mem_type) {
      ...
      case (CUDA_MALLOC_PAGE):
      {
        ...
        DLOG(ERROR, "🤤cnmjklnkljn (%d). original data \n", *((VA)lhpages_addr+count));
        // DLOG(ERROR, "🤤cnmjklnkljn (%p). original data addr\n", ((VA)lhpages_addr+count));
        DLOG(ERROR, "🤤cnmjklnkljn (%p). gpu dest_addr \n", dest_addr);
        DLOG(ERROR, "🤤cnm data real size  (%lu). Exiting...\n", size);
        // DLOG(ERROR, "🤤cnm sizeof int (%lu). Exiting...\n", sizeof(int));



        CHECK_ERROR(cudaMemcpy(dest_addr, ((VA)lhpages_addr+count), size, cudaMemcpyHostToDevice)); 



        // cudaMalloc(&dest_addr, size);
        // int *cuda_c = NULL;
        // CHECK_ERROR(cudaMalloc(&cuda_c, size));
        // DLOG(ERROR, "🤤 \n");
        ...
        // int c =1;
        // DLOG(ERROR, "🤤c %d\n",c);
	      // CHECK_ERROR(cudaMemcpy(&c, cuda_c, size, cudaMemcpyDeviceToHost));
        // DLOG(ERROR, "🤤cnm c val=(%d).test a->gpu->c \n", c);
        
        // 尝试cudaMemcpy 另行执行
        	// int *cuda_c = NULL;
        // cudaMalloc(&cuda_c, sizeof(int));
          // int a = 2;
          // cudaMemcpy(cuda_c, &a , sizeof(int), cudaMemcpyHostToDevice);
          // int c = 3;
        ...
        // DLOG(ERROR, "cnmjklnkljn (%d). Exiting...\n", dest_addr);
        
        // int * h_data = NULL;
        // h_data = (int *)malloc(size);
        // *h_data = 1;
	      // cudaMemcpy(h_data, dest_addr, size, cudaMemcpyDeviceToHost);
        // cudaDeviceSynchronize();
        // DLOG(ERROR, "🤤cnmjklnkljn (%d). Exiting...\n", *h_data);
