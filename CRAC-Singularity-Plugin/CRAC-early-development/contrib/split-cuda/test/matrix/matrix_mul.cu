...
void copyMatricesToGPU() {
	...
	printf("👿A_GPU address = %p \n", A_GPU);
	printf("👿B_GPU address = %p \n", B_GPU);
...
void copyResultFromGPU() {
	...
	printf("👿gputocpu C_CPU[0],[1]= %f,%f \n", C_CPU[0],C_CPU[1]);
	printf("👿C_GPU address = %p \n", C_GPU);
...
void computeCpuMMM() {
	...
	printf("👿计算没有错误  cpu上单独计算矩阵乘法  cpu c val %f,%f\n",  C[0],C[1]);
}
...
__global__ void computeGpuMMM_primitive(float *A_GPU, float *B_GPU, float *C_GPU, int width, int B_D2){
		...
		if((row*B_D2+col) == 0){
		printf("👿gpu kernal out C_GPU[0] %f ,%f\n", result,C_GPU[0]);
		} 
}
